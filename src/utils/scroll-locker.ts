...
// 工具函数
function preventDefault(event: TouchEvent, isStopPropagation: boolean): void {
...
function getScrollParent(el: HTMLElement, root: Window | HTMLElement = window): HTMLElement | Window {
...
function createScrollLocker(): ScrollLocker {
  ...
  const onTouchMove = (event: TouchEvent): void => {
    ...
    if (
      status !== '11'
      && isVertical()
      && !(Number.parseInt(status, 2) & Number.parseInt(moveDirection, 2))
    ) {
